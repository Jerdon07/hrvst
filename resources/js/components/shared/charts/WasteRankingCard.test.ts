...
vi.mock('@inertiajs/vue3', () => ({
    usePage: () => ({ props: { auth: { user: { get roles() { return mockRoles } } } } }),
    Link: { template: '<a :href="href.url ?? href"><slot /></a>', props: ['href'] },
}))

vi.mock('@/routes/admin/vegetables', () => ({
    show: (args: { vegetable: number }) => ({ url: `/admin/vegetables/${args.vegetable}` }),
}))

vi.mock('@/routes/vegetables', () => ({
    show: (args: { vegetable: number }) => ({ url: `/vegetables/${args.vegetable}` }),
}))

function item(id: number, value_kg: number, extra: Record<string, unknown> = {}) {
    return {
        id,
        display_name: `Veg ${id}`,
        image_url: '',
...
describe('WasteRankingCard', () => {
    ...
    it('shows the empty state when items is an empty array', () => {
        const wrapper = mount(WasteRankingCard, { props: { ...baseProps, items: [] } })
        expect(wrapper.text()).toContain('No data available for this period.')
    })
    ...
    it('truncates to initialVisible and exposes a "Show N more" toggle', async () => {
        const items = [item(1, 10), item(2, 20), item(3, 30), item(4, 40), item(5, 50)]
    ...
    it('sizes bars relative to the max value_kg in the list — largest item is 100%', () => {
        ...
        const widths = wrapper
            .findAll('li')
            .map((li) => (li.find('div.rounded-full[style]').element as HTMLElement)?.style.width)
    ...
    it('labels "developing" confidence as "Building history", and treats any other value as no label', () => {
        const developing = mount(WasteRankingCard, {
            props: { ...baseProps, items: [item(1, 10, { confidence: 'developing' })] },
        })
        const mature = mount(WasteRankingCard, {
            props: { ...baseProps, items: [item(1, 10, { confidence: 'mature' })] },
        })
